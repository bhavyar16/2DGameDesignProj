  ...
  channels() 
{
  ...
  music = Mix_LoadMUS("sound/bg.wav");
  // Need to install midi to play the following:
  // music = Mix_LoadMUS("sound/ballad2.mid");
  if (!music) throw string("Couldn't load bg.wav")+Mix_GetError();

  startMusic();
  sounds.push_back( Mix_LoadWAV("sound/swoosh3.wav") );
  sounds.push_back( Mix_LoadWAV("sound/guard.wav") );
